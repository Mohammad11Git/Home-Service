...
from .spectacular import (
    ListOrdersSpectacular,
    MakeOrderSpectacular,
    SellerCommentSpectacular,
...
class MyOrders(APIView):
    ...
    @extend_schema(responses={200: ListOrdersSpectacular(many=True), 401: None})
    def get(self, request):
        ...
        for i, order in enumerate(queryset):
            ...
            serializer.data[i]["client"]["photo"] = (
                "http://" + request.get_host() + order.client.user.photo.url
            )

            serializer.data[i]["seller"] = dict()
            serializer.data[i]["seller"][
                "username"
            ] = order.home_service.seller.user.username
            serializer.data[i]["seller"][
                "first_name"
            ] = order.home_service.seller.user.first_name
            serializer.data[i]["seller"][
                "last_name"
            ] = order.home_service.seller.user.last_name
            serializer.data[i]["seller"]["photo"] = (
                "http://"
                + request.get_host()
                + order.home_service.seller.user.photo.url
            )
...
@extend_schema(request=CreateHomeServiceSerializer)
...
@extend_schema(responses={403: None, 204: None, 401: None})
...
class UpdateFormHomeService(APIView):
    ...
    @extend_schema(responses={200: InputFieldSerializer(many=True)})
    ...
    @extend_schema(
        ...
        responses={
            200: InputFieldSerializer(many=True),
            400: None,
            403: None,
            401: None,
        },
...
class MakeOrderService(APIView):
    ...
    @extend_schema(
        request=MakeOrderSpectacular, responses={200: None, 400: None, 401: None}
    )
...
class CancelOrder(APIView):
    ...
    @extend_schema(responses={401: None, 204: None, 404: None, 403: None})
...
class RejectOrder(APIView):
    ...
    @extend_schema(responses={204: None, 401: None, 403: None, 404: None})
...
@extend_schema(
    responses={
        ...
        403: None,
        404: None,
        200: RetrieveInputDataSerializer(many=True),
        400: None,
    }
)
...
@extend_schema(responses={200: None, 400: None, 401: None, 404: None, 403: None})
...
class ListRatingsByService(APIView):
    def get(self, request, service_id):
        ...
        for rate in ratings:
            ...
            serializer.data[index]["client"][
                "first_name"
            ] = rate.order_service.client.user.first_name
            serializer.data[index]["client"][
                "last_name"
            ] = rate.order_service.client.user.last_name
            serializer.data[index]["client"][
                "username"
            ] = rate.order_service.client.user.username
            serializer.data[index]["client"]["photo"] = (
                "http://"
                + request.get_host()
                + rate.order_service.client.user.photo.url
            )
            index += 1
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def get(self, request, username):
        ...
        for rate in ratings:
            ...
            serializer.data[index]["home_service"][
                "id"
            ] = rate.order_service.home_service.id
            serializer.data[index]["home_service"][
                "title"
            ] = rate.order_service.home_service.title
            serializer.data[index]["home_service"]["category"] = str(
                rate.order_service.home_service.category
            )
            index += 1
        ...
        if queryset.count() > 0:
            for earning in queryset:
                ...
                if earning.order:
                    ...
                    seller_full_name = (
                        str(earning.order.home_service.seller.user.first_name)
                        + " "
                        + str(earning.order.home_service.seller.user.last_name)
                    )
                else:

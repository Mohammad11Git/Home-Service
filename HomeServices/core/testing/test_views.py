...
class TestCoreAPIViews(TestCase):
    def setUp(self):
        ...
        self.global_user = mixer.blend(
            NormalUser, user__mode="seller", user__gender="Male"
        )
    ...
    def test_update_profile_put_request(
        self, bio="bio", first_name="first_name", last_name="last_name"
    ):
        data = {
            "bio": bio,
            "user": {
    ...
    def test_confirm_email_success(self):
        ...
        response = self.client.post(
            url, data={"email": user.email, "confirmation_code": 123456}
        )
        print(response.json())
    ...
    def test_confirm_email_errors_class(self):
        ...
        response2 = self.client.post(
            url, data={"email": "invalid email", "confirmation_code": 123456}
        )
        response3 = self.client.post(
            url,
            data={"email": self.global_user.user.email, "confirmation_code": 123456},
        )
        response4 = self.client.post(url, data={"email": self.global_user.user.email})
        response5 = self.client.post(
            url, data={"email": "blablabla@bla.bla", "confirmation_code": 123456}
        )
    ...
    def test_charge_balance_error_no_enough_money(self):
        ...
        response = self.client.post(
            url, {"username": user.user.user.username, "charged_balance": 100}
        )
